    ...
    */
    ...
    function canWalkHere(x, y) {
        return ((world.get(x,y) != null) &&
            (world.get(x,y) <= maxWalkableTileNum));
    };
            ...
            } else // not the destination
            {
                ...
                for (i = 0, j = myNeighbours.length; i < j; i++) {
                    ...
                    if (!AStar[myPath.value]) {
                        //power cost
                        var pcost = world.get(myNeighbours[i].x, myNeighbours[i].y)- world.get(myNode.x, myNode.y);
                        // estimated cost of this particular route so far
                        myPath.g = myNode.g + distanceFunction(myNeighbours[i], myNode) + pcost;
